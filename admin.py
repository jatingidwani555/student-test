...
def clean_dataframe_columns(df):
    """
    Defensively cleans and normalizes dataframe columns.
    Converts columns to lowercase and extracts the base column name (removing any table/alias prefixes).
    Also, removes duplicates if any are generated by identical base names.
    """
    ...
    for col in df.columns:
        ...
        base_name = col_str.split('.')[-1]
        
        # If we have already seen this base_name, suffix it or skip it to avoid duplicates
        if base_name in seen:
            i = 1
            temp_name = f"{base_name}_{i}"
...
def show_admin_dashboard():
    ...
    with tab_overview:
        ...
        if total_attempts > 0:
            ...
            df_res = clean_dataframe_columns(df_res)
            if 'percentage' not in df_res.columns:
                df_res['percentage'] = 0.0
            if 'exam_title' not in df_res.columns:
                df_res['exam_title'] = 'Unknown Exam'
    ...
    with tab_exams:
        ...
        with col_list:
            ...
            if exams_list:
                ...
                df_exams = clean_dataframe_columns(df_exams)
                
                # Defensive column parsing
                cols_to_use = ['id', 'title', 'duration_minutes', 'question_count', 'total_marks', 'created_at']
                existing_cols = [col for col in cols_to_use if col in df_exams.columns]
                df_exams = df_exams[existing_cols].copy()
                ...
                rename_mapping = {col: col_mapping[col] for col in existing_cols if col in col_mapping}
                df_exams.rename(columns=rename_mapping, inplace=True)
    ...
    with tab_manual_q:
        ...
        if not active_exams:
            ...
            with st.form("manual_question_form"):
                ...
                col_c_m1, col_c_m2 = st.columns(2)
                with col_c_m1:
    ...
    with tab_students:
        ...
        with col_stud_list:
            ...
            if students_list:
                ...
                df_studs = clean_dataframe_columns(df_studs)
                
                # Defensively populate missing columns if any
                if 'username' not in df_studs.columns:
                    df_studs['username'] = 'N/A'
                if 'full_name' not in df_studs.columns:
                    df_studs['full_name'] = 'N/A'
                
                df_disp_studs = df_studs[['username', 'full_name']].copy()
                df_disp_studs.columns = ['Username', 'Full Name']
                st.dataframe(df_disp_studs, use_container_width=True, hide_index=True)
                
                # Delete Student Form
                st.markdown("---")
                delete_target = st.selectbox("Select Student to Remove", df_disp_studs['Username'].tolist())
    ...
    with tab_reports:
        ...
        if results_list:
            ...
            df_results = clean_dataframe_columns(df_results)
            
            # Format display and handle missing columns defensively
            required_res_cols = ['username', 'full_name', 'exam_title', 'score', 'total_marks', 'percentage', 'completed_at']
            fallback_map = {
                'username': 'N/A',
                'full_name': 'N/A',
                'exam_title': 'N/A',
            ...
            for col in required_res_cols:
                if col in df_results.columns:
                    df_disp[col] = df_results[col]
                else:
                    df_disp[col] = fallback_map[col]
                    
            df_disp.columns = ['Username', 'Student Name', 'Exam Title', 'Score', 'Total Marks', 'Percentage (%)', 'Date Completed']
            
            # Dynamic filtering
            unique_exams = []
            if 'exam_title' in df_results.columns:
                unique_exams = df_results['exam_title'].dropna().unique().tolist()
            filter_exam = st.selectbox("Filter Report by Exam", ["All Exams"] + unique_exams)
